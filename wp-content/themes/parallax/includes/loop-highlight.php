...
<article itemscope itemtype="http://schema.org/Article" id="highlight-<?php the_ID(); ?>" <?php post_class('post highlight-post ' . $themify->col_class); ?>>
	...
	if ( $check != $themify->hide_image ):
		...
		<figure class="post-image <?php echo $no_chart; ?>">
			...
			<?php else:
				themify_image('ignore=true&w='.$themify->width.'&h='.$themify->height . '&image_meta=true');
			endif;
			?>
	...
	<div class="post-content">
		<?php if ( $check != $themify->hide_title ): ?>
			<h4 class="post-title entry-title" itemprop="headline"><?php echo $before; ?><?php the_title(); ?><?php echo $after; ?></h4>
