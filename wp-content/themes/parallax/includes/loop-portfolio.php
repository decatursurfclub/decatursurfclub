...
<article itemscope itemtype="http://schema.org/Article" id="post-<?php the_ID(); ?>" <?php post_class("post clearfix portfolio-post" . $themify->theme->get_categories_as_classes(get_the_ID())); ?>>
	...
	<?php
	if ( ! isset( $themify->feature ) ) {
		$themify->feature = 'gallery';
	}
	if ( themify_check( 'gallery_shortcode' ) && 'yes' != $themify->hide_image && 'gallery' == $themify->feature ) { ?>
	...
	<?php } elseif( $post_image = themify_get_image('ignore=true&'.$themify->auto_featured_image . $themify->image_setting . "w=".$themify->width."&h=".$themify->height . '&image_meta=true') ) { ?>
