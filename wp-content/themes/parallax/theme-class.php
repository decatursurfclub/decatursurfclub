...
class Themify_ThemeClass {
	...
	function __construct() {
		...
		add_filter( 'template_include', array( $this, 'porto_expand_content' ) );
	...
	function hide_view_post($return, $id, $new_title, $new_slug){
		if(in_array( get_post_type( $id ), array(self::$section, self::$highlight, self::$team))) {
	...
	function register_post_types() {
		...
		foreach( $cpts as $key => $cpt ){
			register_post_type( $key, apply_filters(
				'themify_theme_register_' . $key . '_post_type',
				array(
					'labels' => array(
						'name' => $cpt['plural'],
					...
					'rewrite' => array(
						'slug' => isset( $cpt['rewrite'] ) ? $cpt['rewrite']: sanitize_title( $cpt['singular'] ),
						'with_front' => false,
					),
					'query_var' => true,
					'can_export' => true,
	...
	function custom_header_background( $classes = '' ) {
		...
		$background_type = get_post_meta( $post_id, 'background_type', true );
		if( $gallery = ( $background_type == 'slider' || ( $background_type  == '' && get_post_meta( $post_id, 'background_gallery', true ) != '' ) ) ) {
		...
		if ( ! empty( $video ) && 'video' == $background_type ) {
	...
	function show_video() {
		global $wp_embed;
		echo $wp_embed->run_shortcode('[embed]' . themify_https_esc( themify_get( 'video_url' ) ) . '[/embed]');
	}
	...
	function porto_expand_link($link) {
		global $post, $themify;
		if( 'yes' == $themify->portfolio_expander
	 ...
	 * @return void
	 */
	function porto_expand_content( $original_template ) {
		if( is_single() && isset( $_GET['porto_expand'] ) && $_GET['porto_expand'] == 1 ) {
			...
			$templatefilename = 'single-portfolio-expand.php';
			return locate_template( $templatefilename );
		} else {
